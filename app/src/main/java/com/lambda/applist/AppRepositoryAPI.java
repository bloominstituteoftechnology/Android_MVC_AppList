...
public class AppRepositoryAPI {


    private static ArrayList<AppListing> dataSet= new ArrayList(1000);


    public static ArrayList<AppListing> getAllAppListing() {
        //populateDrugData();
        if(dataSet == null) {
            populateAppListing();
        }
    ...
    public static ArrayList<AppListing>getByPartialAppName(String target){
        populateAppListing();
        if(dataSet==null){

        }

        ArrayList<AppListing> results=new ArrayList<>();
        for(AppListing apl :dataSet){
            if(apl.getAppName().toString().toLowerCase().equals(target.toLowerCase())){
                results.add(apl);
            }
        }
        return results;
    ...
    private static void populateAppListing() {
        parseRawData(APP_DATA);

    }

    public static AppListing getAppName(String target){
        if(dataSet==null){
            populateAppListing();
        }
    ...
    private static void parseRawData(String stringData) {

        String[]   lines  = stringData.split("\n");
        String[][] dataValues = new String[lines.length][];
        String stringEach[]=new String[6];
        for (int i =1; i < lines.length; ++i) {

            stringEach= lines[i].split(",");
            AppListing ap =new AppListing(stringEach);
            ap.addByString(stringEach);
            dataSet.add(ap);
    ...
    final static String APP_DATA="id,app_name,app_version,domain_name,contact_email,image_url\n" +
